...
P1 tools are pinned with official release URLs and SHA-256 values in `p1-toolchain.json`; install them with `scripts/pipeline-p1-setup`. `pipeline-with-sccache` accelerates Cargo when possible and transparently falls back to Cargo without a wrapper. Local entry points are `scripts/pipeline-security`, `scripts/pipeline-playwright test`, and `scripts/pipeline-maestro`. The Maestro command always validates flow structure and executes on an authorized `adb` device when one is present. P1 policy, commands, findings and limitations are recorded in `resume/P1_USER_TESTS.md`.
